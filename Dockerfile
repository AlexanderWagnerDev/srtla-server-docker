...
RUN git clone -b v0.51.0 https://github.com/yhirose/cpp-httplib.git cpp-httplib && \
...
RUN git clone -b 1.5.2 https://github.com/OpenIRL/srt-live-server.git srt-live-server && \
    cd srt-live-server && \
    make -j$(nproc)
...
RUN git clone -b 1.0.0 https://github.com/OpenIRL/srtla.git srtla && \
    cd srtla && \
